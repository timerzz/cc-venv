...
func GetLLMConfig(c *gin.Context) {
	...
	if err == nil {
		// API Key 脱敏显示
		if apiKey := firstNonEmpty(envVars["ANTHROPIC_AUTH_TOKEN"], envVars["ANTHROPIC_API_KEY"]); apiKey != "" {
			cfg.APIKey = maskAPIKey(apiKey)
		}

		cfg.BaseURL = envVars["ANTHROPIC_BASE_URL"]
		cfg.Models.Default = firstNonEmpty(envVars["ANTHROPIC_MODEL"], envVars["ANTHROPIC_SMALL_FAST_MODEL"])
...
func UpdateLLMConfig(c *gin.Context) {
	...
	// 使用 Claude Code 原生支持的环境变量
	if req.APIKey != "" {
		envVars["ANTHROPIC_AUTH_TOKEN"] = req.APIKey
	...
	if req.Models.Default != "" {
		envVars["ANTHROPIC_MODEL"] = req.Models.Default
		delete(envVars, "ANTHROPIC_SMALL_FAST_MODEL")
	}
